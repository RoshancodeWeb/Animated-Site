...
const Navbar = () => {
  ...
  return (
    <motion.div  variants={navAnimation} initial="initial" animate="animate" transition="transition"
     className='w-full px-20 fixed z-50 bg-black/20 py-5 font-["Neue Montreal"] flex  justify-between items-center '>
        <div className=' logo'>
        ...
        <div className='links flex gap-5'>
           {["Services","Our Work","About Us","Insights","Contact"].map((value,index)=>(
             <p   key={index}   className={`text-md font-light cursor-pointer hover:scale-110 transition-transform  ease-linear duration-150 ${index=== 4 && "ml-20"}`}>{value}</p>
           ))}
        </div>
